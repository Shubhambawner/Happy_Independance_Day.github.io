...
timeBox = document.getElementById("clock");
...
setInterval(() => {
  ...
  min = Dt.slice(19, 21);
  sec = Dt.slice(22, 24);
  timeBox.innerHTML = `<h2> ${hr}: ${min}:${sec} </h2>`;
}, 1000);
...
function valid(IPdate, IPtime) {
  ...
  if (new Date("2021-12-31" + "T" + IPtime) == "Invalid Date") {
    alert(
      "Time format entered is  invalid !!!\nplease follow the input format, machines are dumb"
    );
    return false;
  } else {
    return true;
...
icon = document.querySelector('.wrapper header i');


icon.addEventListener("click", ()=>{
    td.addClassList.remove('');
})
